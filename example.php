<?php
/**
 * Contoh Script Bridge API Flickshort via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Flickshort di sistem Bridge
function callFlickshortAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/flickshort/api/v1";

// Contoh 1: Fitur Cari Film Flickshort
echo "\n=====================[ 1. Pencarian Flickshort ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callFlickshortAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callFlickshortAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callFlickshortAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/flickshort ◀◀◀\n\n";
